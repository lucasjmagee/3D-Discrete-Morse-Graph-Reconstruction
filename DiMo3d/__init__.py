...
def __single_merge(raw_dir, merge_dir, first_dir, persistence_threshold, merge_threshold, d):
    ...
    with open(config_filename, 'r') as config_file:
        ...
        for row in reader:
            if count < 2:
                count += 1
                continue
            merged_morse_dirs.append(os.path.join(raw_dir, row[0] + '/' + str(thresh) + '/'))
            count += 1

    final_vert_filename = os.path.join(morse_dir, 'dimo_vert.txt')
    final_edge_filename = os.path.join(morse_dir, 'dimo_edge.txt')
    ...
    merged_vert_filename = os.path.join(morse_dir, 'overlap_dimo_vert.txt')
    merged_edge_filename = os.path.join(morse_dir, 'overlap_dimo_edge.txt')

    with open(merged_vert_filename, 'r') as vert_file:
        reader = csv.reader(vert_file, delimiter=' ')
        for row in reader:
            final_verts.append([int(row[0]), int(row[1]), int(row[2]), float(row[3])])
            vert_dict[(int(row[0]), int(row[1]), int(row[2]))] = (local_count, float(row[3]))
            local_count += 1
        vert_file.close()
        v_shift += local_count
    ...
    for d in merged_morse_dirs:
        ...
        with open(vert_filename, 'r') as vert_file:
            reader = csv.reader(vert_file, delimiter=' ')
            for row in reader:
                local_verts.append([int(row[0]), int(row[1]), int(row[2]), float(row[3])])
            vert_file.close()

        with open(edge_filename, 'r') as edge_file:
            reader = csv.reader(edge_file, delimiter=' ')
            for row in reader:
                local_edges.append([int(row[0]), int(row[1])])
            edge_file.close()
        ...
        count = 0
        for v in range(len(local_verts)):
            if v not in valid_verts:
                continue
            used_vert_dict[v] = count
            used_verts.append(local_verts[v])
            count += 1

        for e in local_edges:
            e[0] = used_vert_dict[e[0]]
            e[1] = used_vert_dict[e[1]]

        local_count = 0
        dup = 0
        for v in used_verts:
            if (v[0], v[1], v[2]) in vert_dict.keys():
                local_vert_dict[local_count] = vert_dict[(v[0], v[1], v[2])][0]
                vert_dict[(v[0], v[1], v[2])] = (vert_dict[(v[0], v[1], v[2])][0], v[3])
                dup += 1
                # match += 1
            else:
                local_vert_dict[local_count] = v_shift + local_count - dup
                final_verts.append(v)
                assert(len(final_verts) == v_shift + local_count - dup + 1)
            local_count += 1
            # total += 1
    ...
    with open(final_vert_filename, 'w') as vert_file:
        for v in final_verts:
            vert_file.write(str(v[0]) + ' ' + str(v[1]) + ' ' + str(v[2]) + ' ' + str(v[3]) + '\n')
        vert_file.close()

    with open(final_edge_filename, 'w') as edge_file:
        for e in final_edges:
            edge_file.write(str(e[0]) + ' ' + str(e[1]) + '\n')
        edge_file.close()


def __execute_merging(input_dir, output_dir, first_dir, persistence_threshold, merge_threshold, threads=1):
    ...
    pool = Pool(threads)
    pool.map(partial(__single_merge, input_dir, output_dir, first_dir, persistence_threshold, merge_threshold), valid)
    pool.close()
    pool.join()
...
def merge(input_path, merge_dir, persistence_threshold, merge_threshold, nx, ny, nz, x_len, y_len, z_len, overlap, threads=1):
    ...
    while len([listing for listing in os.listdir(current_dir) if os.path.isdir(os.path.join(current_dir, listing))]) > 1:
        ...
        __execute_merging(current_dir, next_output_dir, input_path, persistence_threshold, merge_threshold, threads=threads)
...
def write_vtp_graph(vert_filename, edge_filename, output_filename):
    vert = np.loadtxt(vert_filename)
    edge = np.loadtxt(edge_filename)
...
def __single_subregion(input_dir, output_dir, x_min, x_max, y_min, y_max, filename):
    ...
    input_img = input_img.astype('uint16')
    print('outputting image')
    cv2.imwrite(output_filename, input_img)


def extract_subregion(input_dir, output_dir, x_center, y_center, z_center, x_len, y_len, z_len, threads=1):
    ...
    x_min = int(x_center / .35) - x_half
    x_max = int(x_center / .35) + x_half
    y_min = int(y_center / .35) - y_half
    y_max = int(y_center / .35) + y_half
    z_min = int(z_center) - z_half
    z_max = int(z_center) + z_half

    f_ImageId = [f for f in os.listdir(input_dir)]
    f_ImageId.sort()
    valid = []
    index = 0
    for filename in f_ImageId:
        # print(index)
        if index < z_min:
            index += 1
            continue
    ...
    p = Pool(threads)
    p.map(partial(__single_subregion, input_dir, output_dir, x_min, x_max, y_min, y_max), valid)
    p.close()
    p.join()
