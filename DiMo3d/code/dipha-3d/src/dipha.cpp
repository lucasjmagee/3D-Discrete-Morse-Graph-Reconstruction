...
*/
void parse_command_line(int argc, char** argv, bool& benchmark, bool& dualize, int64_t& upper_dim, std::string& input_filename, 
                        std::string& output_filename, std::string& edge_filename)
{
  ...
  input_filename = argv[argc - 3];
  output_filename = argv[argc - 2];
  edge_filename = argv[argc - 1];
...
void compute(const std::string& input_filename, bool dualize, int64_t upper_dim, const std::string& output_filename, const std::string& edge_filename)
{
  Complex complex;
  ...
  std::vector<int64_t> lattice_res = complex.get_lattice_resolution();
  int64_t nx = lattice_res[0];
  int64_t ny = lattice_res[1];
  int64_t nz = lattice_res[2];
  ...
  //DIPHA_MACROS_BENCHMARK(dipha::outputs::save_persistence_diagram(output_filename, edge_filename, complex, filtration_to_cell_map, reduced_columns, 
                                                                  dualize, upper_dim); );
}
...
int main(int argc, char** argv)
{
  ...
  parse_command_line(argc, argv, benchmark, dualize, upper_dim, input_filename, output_filename, edge_filename);
  ...
  case dipha::file_types::IMAGE_DATA:
    ...
    compute< dipha::inputs::weighted_cubical_complex >(input_filename, dualize, upper_dim, output_filename, edge_filename);
    break;
  default:
