...
void Simplicial2Complex::build_spt3(float sigma){
    ...
    for(auto it=edgeList.begin();it!=edgeList.end();it++){
        if((*it).critical_type==0){
            ...
            Edge *cri_e = atE((*it).getEPosition());
            int *e_vert = cri_e->getVertices();
            int v0 = e_vert[0];
            int v1 = e_vert[1];
            //edgefile << v0 << " " << v1 << " 2 inf" << endl;
        }
    }
	...
	*/
	...
	for(unordered_set<Simplex*>::iterator it = cBegin(); it != cEnd(); it++){
		...
		if(s->dim == 1){
			...
			if (e->critical_type == 1 && e->persistence < et_delta + EPS_compare) continue;
...
void Simplicial2Complex::buildComplexFromFile2_BIN(string pathname) {
	...
	for (int i = 0; i < numOfVertices; i++) {
		...
		if (min_x == -1 or coords[0] < min_x)
		{
			min_x = coords[0];
		}
		if (max_x == -1 or coords[0] > max_x)
		{
			max_x = coords[0];
		}
		if (min_y == -1 or coords[1] < min_y)
		{
			min_y = coords[1];
		}
		if (max_y == -1 or coords[1] > max_y)
		{
			max_y = coords[1];
		}
		if (min_z == -1 or coords[2] < min_z)
		{
			min_z = coords[2];
		}
		if (max_z == -1 or coords[2] > max_z)
		{
			max_z = coords[2];
		}
	...
	std::cout << "Coord bounds: " << min_x << " " << max_x << " " << min_y << " " << max_y << " " << min_z << " " << max_z << std::endl;
        //return;	
...
//	}
	...
	*int_writer = num_et;
	...
	if (DEBUG){
		ofstream ppairs("etPairs.txt", ios_base::trunc | ios_base::out);
		for(auto pp = P.smBegin(); pp != P.smEnd(); ++pp){
                PersistencePairs::write_et_pair_debug(*pp, ppairs);
		}
        ppairs.close();
    
    if (DEBUG){
        ofstream ppairs_ve("vePairs.txt", ios::binary);
        for(auto pp = P.msBegin(); pp != P.msEnd(); ++pp){
            PersistencePairs::write_ve_pair_debug(*pp, atV(pp->min)->getoriPosition(), ppairs_ve);
        }
        ppairs_ve.close();
    }
//        ofstream ppairs2("vePairs.txt", ios_base::trunc | ios_base::out);
//        for(auto pp = P.msBegin(); pp != P.msEnd(); ++pp){
//            pp->msPersistencePairs
//            PersistencePairs::write_ve_pair_debug(*pp, ppairs);
//        }
//        ppairs.close();
	}
